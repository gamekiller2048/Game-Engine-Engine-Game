...
namespace mgl
{
    ShaderVariant::ShaderVariant(uint version) :
        version(version) {}
...
{    
    ...
    StandardShaderVariant::StandardShaderVariant(RenderPipeline pipeline, bool diffuseMap, bool normalMap, bool specularMap, uint numDirLights, uint numPointLights, uint version) :
        ShaderVariant(version), pipeline(pipeline), diffuseMap(diffuseMap), normalMap(normalMap), specularMap(specularMap), numDirLights(numDirLights), numPointLights(numPointLights)
    {
        std::string defines;
        std::string baseVert;
        std::string baseFrag;
        ...
        std::string versionStr = versionString(version, true);
        srcVert = versionStr + defines + baseVert;
        srcFrag = versionStr + defines + baseFrag;
    }
...
void main()
{
    gl_Position = vec4(v_pos, 1) * u_transform * u_projection;
})";
...
void main()
{
    ...
    BasicShaderVariant::BasicShaderVariant(uint version) :
        ShaderVariant(version)
    {
        std::string versionStr = versionString(version, true);
        srcVert = versionStr + basicVert;
        srcFrag = versionStr + basicFrag;
    }
}