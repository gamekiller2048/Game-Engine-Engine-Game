...
namespace mgl
{
    static std::unordered_map<MemoryBarrier, gl::MemoryBarrier> memoryBarrierMap = {
        {MemoryBarrier::ALL, gl::MemoryBarrier::ALL}
    };
    ...
    {
        impl->program.attachShadersFromFile(vertexPath, fragmentPath, geometryPath, computePath);
    }
    ...
    void GLShaderProgram::compute(const mml::uvec3& workGroups) const
    {
        impl->program.compute(workGroups);
    }

    void GLShaderProgram::computeBarrier(MemoryBarrier barrier) const
    {
        impl->program.computeBarrier(memoryBarrierMap[barrier]);
    }
